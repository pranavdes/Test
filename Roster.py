...
def get_working_dates(year, month, public_holidays):
    """Returns a sorted list of working dates (datetime objects) for the given month/year,
...
def parse_days_string(days_str):
    ...
    both the 3-letter abbreviation and full name (all lowercase).
...
def parse_day_descriptor(descriptor):
    ...
    and weekday is normalized (e.g., "mon"). Returns (None, None) if parsing fails.
    """
...
def is_day_descriptor_match(date_obj, descriptor, working_dates):
    """
    Returns True if date_obj (a working date) matches the descriptor (e.g., "1st Tuesday" or "Last Fri")
    based on the list of working_dates.
    """
...
# ------------------------------
# Global ILP Rostering Solver (Enhanced)
# ------------------------------
...
def generate_roster_schedule_ilp(excel_file, designated_min=3, big_penalty=1000):
    ...
    Builds and solves a global ILP model that assigns seats over the month so that:
      1. Every employee meets their overall quota (if feasible).
      2. Fixed seats are forced.
      3. On each day, available seats are filled.
      4. On designated days (from SubTeamOfficeDays) employees receive a bonus, and a soft
         constraint (with slack) forces each employee to get at least 'designated_min'
         assignments on designated days.
      5. On special days (from SpecialSubTeamDays) extra bonus is given for assignments to employees
         in the designated sub-team—but extra assignments on non‑special days are not allowed for staff
         without fixed seats.
         
    In particular, for staff with no fixed-seat assignments (F), we add a constraint so that their total
    assignments on days that are not special for their sub-team do not exceed the threshold.
    
    The final output is written in a column-oriented sheet (named by TargetMonthYear) where:
    ...
    # Read named cells
    office_percentage = get_named_cell_value(wb, "OfficePercentage")  # value in [0,1]
    ...
    # Overall required days for each employee (threshold)
    req_days = {e: round(total_wd * office_percentage) for e in employees}
    
    # Map employee details (output name only) and sub-team membership
    ...
    # Determine designated days per sub-team from SubTeamOfficeDays
    designated_map = {}
    ...
    # Determine special days from SpecialSubTeamDays (if any)
    special = {}  # special[d] = sub-team for which d is special
    for d in working_dates:
        for _, row in df_special_days.iterrows():
            ...
            if is_day_descriptor_match(d, desc, working_dates):
                ...
                break  # assume one special per day
    ...
    for _, row in df_seat_pref.iterrows():
        e = row["EmployeeID"]
        s = row["SeatCode"]
        pref_bonus[(e,s)] = 10  # adjust as needed
    
    # Other bonus parameters
    fill_bonus = 1
    designated_bonus = 5   # bonus if assignment on designated day
    special_bonus = 20     # bonus if on special day and employee is in that sub-team
    
    # Determine seat availability: for each seat s and day d, based on SeatData["Days"]
    seat_avail = {}
    ...
    # Fixed seat assignments: For seats of type "fixed" with an AssignedEmployeeID, force assignment on allowed days.
    fixed = {}
    ...
    # For employees with designated days, create slack variable z_e (integer, >=0)
    z = {}
    for e in employees:
    ...
    # 6. A seat can only be assigned on a day if it is available.
    for s in seats:
        for d in working_dates:
    ...
    # 7. (Special days) – We do not force exclusivity; instead, we add a bonus in the objective.
    # (No additional hard constraint here.)
    
    # 8. New constraint: For employees with no fixed seat assignments, do not assign extra seats on non-special days.
    # Let F = { e in employees such that e never appears as fixed assignment }.
    F = [e for e in employees if e not in set(fixed.values())]
    for e in F:
        # D_ns(e) = { d in working_dates that are NOT special for e's sub-team }
        non_special_days = [d for d in working_dates if not (d in special and emp_subteam[e] == special[d])]
        # Constrain total assignments on non-special days to be at most the threshold.
    ...
    for e in employees:
        ...
        for s in seats:
            for d in working_dates:
                ...
                # Add designated bonus if d is designated for e.
                if d in designated_days[e]:
                    bonus += designated_bonus
                # Add special bonus if d is special and e is in that sub-team.
                if d in special and e_st == special[d]:
                    bonus += special_bonus
    ...
    model += lpSum(obj_terms) - lpSum(penalty_terms), "TotalObjective"
    ...
    # ------------------------------
    # Extract solution: For each employee and day, record the seat code assigned (if any).
    # ------------------------------
    ...
    # ------------------------------
    # Build output sheet (column-oriented):
    # Column A: "Employee Name"
    # Row 1 (columns B onward): Dates (YYYY-MM-DD)
    # Row 2 (columns B onward): Day-of-week (e.g., Mon, Tue)
    # Rows 3 onward: Each employee’s row, with assigned seat code (or blank).
    # ------------------------------
